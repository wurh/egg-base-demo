...
class YamibyService extends Service {
  ...
  async list(page, tab, cat) {
    let pageSize = 20;
    let start = 1 + (pageSize * parseInt(page) - pageSize);
    const {
      serverUrl, category
  ...
  async save(page, tab, cat) {
    ...
    for (let i = 0; i < itemList.length; i++) {
      let item = await this.changToModel(itemList[i], catObj);
  ...
  async queryGoods(page, size, category, brandname, isvipbrand,fliterType) {
    ...
    let orders =  await this.getFliterType(fliterType);
    console.log(orders);
